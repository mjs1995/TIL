...
- 쿠버네티스 : 컨테이너 오케스트레이션의 Defacto
  - 컨테이너를 쉽고 빠르게 배포/확장하고 관리를 자동화해주는 오픈소스 플랫폼
  - 마이크로 서비스, 클라우드 플랫폼을 지향하고 컨테이너로 이루어진 것들을 손쉽게 담고 관리할 수 있는 그릇 역할을 함
  - 서버리스, CI/CD, 머신러닝 등 다양한 기능이 쿠버네티스 플랫폼 위에서 동작함
  - 쿠버네티스 과거,현재
    - 다른 컨테이너 오케스트레이션 도구보다 비교적 늦은 등장, 도커 스웜이 수비고 간단한 사용법을 앞세워 세력을 넓히고 있었고, AWS 에서 ECS, 하시코프에서 Nomad, Mesos를 이용한 Marahon이 발표됨
    - 컨테이너 오케스트레이션의 기능으로 서버를 클러스터링 하여 적당한 서버를 선택, 애플리케이션을 배포하고 부하가 생기면 컨테이너를 늘리는 등 대동소이한 기능을 제공하며 한동안 컨테이너 오케스트레이션 춘추전국시대가 열림
    - 쿠버네티스 등장 이후 1~2년 정도가 지나며 사실상 표준(Defacto)으로 자리잡음. 대규모 컨테이너 관리하던 구글의 노하우와 강력한 확장성, MS, IBM, Red Hat 등 수많은 기업의 참여 및 잘 짜인 설계
  - 쿠버네티스 기본 기능
    - 프로덕션 수준의 컨테이너 오케스트레이션으로 기본 기능을 충실히 제공
    - 상태 관리 : 상태를 선언하고 선언한 상태를 유지/ 노드가 죽거나 컨테이너 응답이 없을 경우 자동으로 복구
    - 스케줄링 : 클러스터의 여러 노드 중 맞는 노드를 찾아 컨테이너를 배치
  ...
  - 쿠버네티스 오브젝트
    ...
    - Pod
      ...
      - 컨테이너를 하나만 사용하는 경우도 반드시 Pod으로 감싸서 관리(애플리케이션 기본 실행단위)
    - Replica Set
      - Pod을 여러 개 (한 개 이상) 복제하여 관리하는 오브젝트
      - Pod을 생성하고 개수를 유지하려면 반드시 ReplicaSet을 사용해야 함
      - ReplciaSet은 복제할 개수, 개수를 체크할 라벨 선택자, 생성할 Pod의 설정값(템플릿)등을 가짐
      - 직접적으로 ReplicaSet을 사용하기보다는 Deployment 등 다른 오브젝트에 의해서 사용되는 경우가 많음
    - Object Spec-YAML
      - 오브젝트의 명세 Spec은 YAML 파일로 정의하고 오브젝트의 종류와 원하는 상태를 입력함
      - 이러한 명세는 생성, 조회, 삭제로 관리할 수 있기 때문에 REST API로 쉽게 노출할 수 있음
      - 접근 권한 설정도 같은 개념을 적용하여 누가 어떤 오브젝트에 어떤 요청을 할 수 있는지 정의 가능함
    - Service
      - 네트워크와 관련된 오브젝트
      - Pod을 외부 네트워크와 연결해주고 여러 개의 Pod을 바라보는 내부 로드밸런서를 생성할 때 사용
      - 내부 DNS에 서비스 이름을 도메인으로 등록하기 때문에 서비스 디스커버리 역할을 함
    - Volume
      - 저장소와 관련된 오브젝트
      - 호스트 디렉토리를 그대로 사용할 수 있고 EBS 같은 스토리지를 동적으로 생성하여 사용할 수도 있음
      - 사실상 인기 있는 대부분의 저장 방식을 지원 
  - 아키텍처
    - <img width="982" alt="image" src="https://github.com/mjs1995/TIL/assets/47103479/07d30b39-6d3b-4d7f-a3f0-8ea8d2a5dabe">
    - 마스터
      - 쿠버네티스 클러스터 전체 제어 시스템
      - API 서버 : 쿠버네티스 모든 명령과 통신을 API 기반으로 수행, API 서버 REST API 제공(명령 처리)
      - ETCD : 쿠버네티스 클러스터 정보를 저장하는 DB 서버, ETCD라는 분산형 Key-Value 상태/설정 정보 저장함
      - 스케줄러 : Pod, 서비스 등 각 리소스 적절한 노드 할당 역할
      - 컨트롤 매니저 : Replica, Service, Volume, Node Controller 등 생성하여 각 노드에 배포 관리
      - DNS : POD 및 서비스 Label로 Mapping 하는 포인트, Pod, Service는 생성/삭제 시 동적 IP로 설정되므로 DNS 필요함
    - 노드
      - 마스터 명령을 받고 실제 워크로드 생성, 서비스하는 컴포넌트
      - Kubelet : 노드 배포, Agent, Master와 통신하면서 명령을 받아 수행, 노드 상태 등 마스터 전달
      - Kube-Proxy : 노드가 들어오는 네트워크 트래픽을 적절한 컨테이너로 로드밸런싱 하며, 나가는 트래픽 프록시, 노드-마스터 간 네트워크 통신 관리
      - Container Runtime: Pod를 통해 배포된 컨테이너를 실행하는 런타임, 보통 도커 사용 가능, rkt/Hyper Container 등 다양한 도구 사용
      - cAdvisor : 각 노드 모니터링 에이전트, 노드 내 가동되는 컨테이너 상태 및 성능 정보 수집하여 마스터 서버의 API 서버로 전달         
